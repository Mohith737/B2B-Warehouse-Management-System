...
4. `GET /health` shows `default_password_warning: true` when `INITIAL_ADMIN_PASSWORD` is still `"REDACTED_SEE_ENV"`.
...
- `GET /health` includes `default_password_warning: true` when the env var matches the fallback value `"REDACTED_SEE_ENV"`.
