...
if ! $compose_cmd exec -T api alembic -c /app/alembic.ini upgrade head; then
  ...
  activate_venv_if_present
  HOST_DATABASE_URL="$(host_db_url)"
  if [ -z "$HOST_DATABASE_URL" ]; then
    echo "ERROR: DATABASE_URL is not configured"
    exit 1
  fi
  DATABASE_URL="$HOST_DATABASE_URL" PYTHONPATH="$PROJECT_ROOT" python3 -m alembic -c backend/alembic.ini upgrade head
fi
...
if ! $compose_cmd exec -T api python /workspace/scripts/seed.py; then
  ...
  DATABASE_URL="$HOST_DATABASE_URL" PYTHONPATH="$PROJECT_ROOT" python3 scripts/seed.py
fi
...
echo ""
echo "Demo Accounts"
echo "  Admin:    admin@stockbridge.com / StockAdmin123!"
echo "  Manager:  manager@stockbridge.com / StockManager123!"
echo "  Staff:    staff@stockbridge.com / StockStaff123!"
echo ""
